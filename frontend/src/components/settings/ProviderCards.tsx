...
const CATEGORY_GROUPS = [
  { key: 'free', label: 'Free / Out-of-the-box', desc: 'Use directly without self-hosting.' },
  { key: 'selfhost', label: 'Self-hosted', desc: 'Requires manual deployment of backend services.' },
  { key: 'thirdparty', label: 'Third-party Services', desc: 'Requires registration on third-party platforms.' },
  { key: 'custom', label: 'Custom', desc: 'Integrate with any API via generic HTTP drivers.' },
]
    ...
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {fields.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)]">No additional configuration required for this service.</p>
          ) : fields.map(field => {
              ...
              <div className="space-y-1.5">
                ...
                {cat === 'custom' && (
                  ...
                  >
                    ...
                    Add Custom {providerType === 'mailbox' ? 'Email' : providerType === 'captcha' ? 'Captcha' : providerType === 'sms' ? 'SMS' : ''} Service
                  </button>
                )}
              </div>
            </div>
