...
export function buildRegistrationOptions(platformMeta: any) {
  ...
  if (supportedModes.includes('mailbox')) {
    options.push({
      ...
      description: `Use ${getOptionLabel('mailbox', identityModeOptions)} to automatically receive captcha and complete registration`,
...
export function buildExecutorOptions(
  ...
  return supportedExecutors.map((executor) => {
    ...
    if (executor === 'protocol') {
      option.description = 'Register directly via protocol without opening a browser'
    ...
    if (executor === 'headless') {
      option.description = identityProvider === 'mailbox'
        ? 'Browser executes automatically in the background, UI is invisible'
        : 'Reuse local browser login status to automatically complete third-party login in the background'
      if (identityProvider === 'oauth_browser' && !reusableBrowser) {
        option.disabled = true
        option.reason = 'Chrome Profile path or Chrome CDP address needs to be configured in global settings first'
      }
      return option
    }

    option.description = 'Browser window will open, but the system still executes automatically with no interaction required'
    return option
  })
}
