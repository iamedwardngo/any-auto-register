...
export default function Register() {
  ...
  const activeTaskStats = task ? [
    ...
    { label: 'Fail', value: String(task.error_count ?? task.errors?.length ?? 0), icon: XCircle },
  ] : []
    ...
    <div className="space-y-4">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_340px]">
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader><CardTitle>Step 1 · Identity Type</CardTitle></CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {form.identity_provider === 'oauth_browser' && (
                <>
                  <Input label="Preferred Login Email (Optional)" k="oauth_email_hint" placeholder="your-account@example.com" />
                  ...
                  <p className="text-xs text-[var(--text-muted)]">
                    When using background browser automation for third-party accounts, it is recommended to configure Chrome Profile or Chrome CDP first to reuse existing browser sessions.
                  </p>
                </>
              )}
            </CardContent>
          ...
          {form.identity_provider === 'mailbox' && (
            <Card>
              <CardHeader><CardTitle>System Email Configuration</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {optionsError && (
                ...
                {mailboxProviderOptions.length > 0 ? (
                  ...
                  <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
                    No email providers currently enabled. Please add and enable a default email provider in settings.
                  </div>
                )}
          ...
          {smsProviderOptions.length > 0 && (
            <Card>
              <CardHeader><CardTitle>SMS Service Configuration</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {optionsError && (
                  <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">
                    {optionsError}
                ...
                <CardContent className="space-y-4">
                  ...
                  {task.status === 'interrupted' && !task.error && (
                    <div className="flex items-center gap-2 text-amber-400">
                      <XCircle className="h-4 w-4" />
                      <span className="text-xs">Task was interrupted after service restart</span>
                    </div>
                  )}
                  {task.status === 'cancelled' && !task.error && (
                    <div className="flex items-center gap-2 text-amber-400">
                      <XCircle className="h-4 w-4" />
                      <span className="text-xs">Task has been cancelled</span>
                    </div>
                  )}
                </CardContent>
              </Card>
