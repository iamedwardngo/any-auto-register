...
export default function Dashboard() {
  ...
  const statCards = [
    ...
    { label: 'Invalid/Expired', value: (stats?.by_display_status?.expired ?? 0) + (stats?.by_validity_status?.invalid ?? 0), icon: XCircle, color: 'text-red-400' },
  ]
                    ...
                    <div>
                      ...
                      <div className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
                        {state?.available === false
                          ? (state?.message || 'Desktop status detection not supported for this platform')
