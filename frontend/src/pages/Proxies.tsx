...
export default function Proxies() {
  ...
  const metricCards = [
    ...
    { label: 'Enable', value: activeCount, icon: ShieldCheck, tone: 'text-emerald-400' },
        ...
        <Card className="bg-[var(--bg-pane)]/60">
          <div className="space-y-4">
            <div>
              ...
              <div className="mt-1 text-sm font-medium text-[var(--text-primary)]">Add proxy or batch import</div>
            </div>
            <textarea
              value={newProxy}
            ...
            <div className="rounded-lg border border-[var(--border-soft)] bg-[var(--bg-pane)]/45 px-3.5 py-2.5 text-xs leading-5 text-[var(--text-secondary)]">
              Supports direct entry of single proxy or multi-line batch import. Region tags will be included for filtering.
            </div>
            ...
            {proxies.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8">
                  <div className="empty-state-panel">The proxy pool is empty. Add one from the left or batch import.</div>
                </td>
              </tr>
            )}
            {proxies.map(p => (
