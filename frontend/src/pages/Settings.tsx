...
const PROVIDER_META: Record<ProviderType, {
  ...
  mailbox: {
    ...
    addDialogHint: 'Choose from email provider catalog',
    usageHint: 'Email services configured here are only used when Identity Type is "System Email". Details, editing, setting default, and deletion can be done directly in rows.',
    ...
    listDescription: (count: number) => `${count}  configurations, supports details, editing, setting default, and deletion.`,
    noAvailableText: 'No email providers available to add',
    availableText: (count: number) => `Remaining  ${count}  email providers available to add`,
    emptyText: 'No email provider configurations found, please add one first.',
  ...
  captcha: {
    ...
    addDialogHint: 'Choose from captcha provider catalog',
    usageHint: 'Protocol mode automatically selects remote solving services in order of enablement; Headless mode uses the current default captcha provider. Details, editing, setting default, and deletion can be done directly within list rows.',
    ...
    listDescription: (count: number) => `${count}  configurations, protocol mode will read available items here in sequence.`,
    noAvailableText: 'No captcha providers available to add',
    availableText: (count: number) => `Remaining  ${count}  captcha providers available to add`,
    emptyText: 'No captcha provider configurations found, please add one first.',
  ...
  sms: {
    ...
    addDialogHint: 'Choose from SMS provider catalog',
    usageHint: 'When the platform requires phone verification, it will use enabled SMS providers to create temporary numbers and fill in SMS verification codes. Details, editing, setting default, and deletion can be done directly within list rows.',
    ...
    listDescription: (count: number) => `${count}  configurations, phone and SMS verification will prioritize the default items here.`,
    noAvailableText: 'No SMS providers available to add',
    availableText: (count: number) => `Remaining  ${count}  SMS providers available to add`,
    emptyText: 'No SMS provider configurations found, please add one first.',
        ...
        return (
          <div key={p.name} className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
            ...
            <div className="space-y-3">
              <div>
                <p className="text-xs text-[var(--text-muted)] mb-2">Execution Channel</p>
              ...
              <div>
                <p className="text-xs text-[var(--text-muted)] mb-2">Identity Type</p>
...
const TABS: { id: string; label: string; icon: any; sections?: any[] }[] = [
  {
    ...
    sections: [{
      ...
      items: [
        { key: 'default_identity_provider', label: 'Default Identity Type' },
        { key: 'default_oauth_provider', label: 'Default Third-party OAuth', placeholder: '' },
        { key: 'default_executor', label: 'Default Execution Channel' },
      ],
    }, {
      section: 'Browser Reuse',
      desc: 'When third-party accounts use headless browser automation, they usually need to reuse the locally logged-in browser.',
      items: [
  ...
  const queryPrice = async () => {
    ...
    try {
      ...
      if (current) {
        ...
        setMessage('No price info found for current service/country')
      }
    } catch (e: any) {
      ...
      <div className="dialog-panel dialog-panel-md flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-4 items-center py-3 border-b border-white/5">
            <label className="text-sm text-[var(--text-secondary)] font-medium">Configuration Name</label>
    ...
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-md flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <div>
            <h2 className="text-base font-semibold text-[var(--text-primary)]">{title}</h2>
            <p className="text-xs text-[var(--text-muted)] mt-0.5">Add a new dynamic provider definition and create the first available configuration simultaneously.</p>
          </div>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]"><X className="h-4 w-4" /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
...
export default function Settings({ embedded, defaultTab }: { embedded?: boolean; defaultTab?: string }) {
  ...
  const loadConfigData = async () => {
    ...
    if (options) {
      ...
      setOptionsError('Failed to load provider metadata. Please restart backend and refresh the page.')
    }
  }
  ...
  const getSelectOptions = (key: string) => {
    ...
    if (key === 'default_oauth_provider') {
      return [
        { label: 'No pre-selection, let the page decide', value: '' },
        ...
        <div className="space-y-4">
          {activeTab === 'platform_caps' ? (
            ...
            <>
              {activeTab === 'register' && (
                <div className="rounded-lg border border-[var(--accent-edge)] bg-[var(--accent-soft)] px-4 py-3 text-sm text-[var(--text-secondary)]">
                  Users only need to understand two things: choosing between "System Email" or "Third-party Account" for Identity Type, and choosing between "Protocol Mode / Headless Automation / Visible Automation" for Execution Channel. Settings here only define default values.
                </div>
              )}
