    ...
    try {
      ...
      if (selection.identityProvider === 'mailbox') {
        if (!defaultMailboxProvider?.provider_key) {
          throw new Error('Default email provider not configured. Please enable an email provider in the settings page first')
        }
    ...
    <div className="dialog-backdrop" onClick={!taskId ? onClose : undefined}>
      <div className="dialog-panel dialog-panel-md flex flex-col"
        ...
        <div className="px-6 py-4 flex-1 overflow-y-auto flex flex-col gap-5">
          {!taskId ? (
            configLoading ? (
              <div className="text-sm text-[var(--text-muted)]">Loading registration configurations...</div>
            ) : (
              <>
                <div>
                  <div className="text-xs uppercase tracking-[0.16em] text-[var(--text-muted)]">Step 1</div>
                  <div className="mt-1 text-sm font-semibold text-[var(--text-primary)]">Select Identity Type</div>
                  <div className="mt-1 text-xs text-[var(--text-muted)]">Choose the identity type for registration based on platform support.</div>
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm font-semibold text-[var(--text-primary)]">Select Execution Channel</div>
                  <div className="mt-1 text-xs text-[var(--text-muted)]">All methods are executed automatically, just different protocol or browser channels.</div>
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3 text-xs text-[var(--text-secondary)]">
                  <div>Identity Type: <span className="text-[var(--text-primary)]">{selectedRegistration?.label || '-'}</span></div>
                  <div className="mt-1">Execution Channel: <span className="text-[var(--text-primary)]">{selectedExecutor?.label || '-'}</span></div>
                  ...
                  {selection.identityProvider === 'oauth_browser' && !reusableBrowser && (
                    <div className="mt-2 text-amber-400">Headless automation relies on Chrome Profile or Chrome CDP. When not configured, only visible automation is allowed.</div>
                  )}
                </div>
        ...
        <div className="relative overflow-hidden border-b border-[var(--border)] px-6 py-5">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-[var(--text-muted)]">Task status, error summary, and real-time logs are displayed here</p>
            </div>
            <div className="flex items-center gap-2">
              {taskStatus ? (
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)] mt-0.5">Fill in the required parameters for this action</p>
          </div>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]">
            <X className="h-4 w-4" />
          </button>
...
function ActionMenu({
  ...
  const handleActionDone = async (status: string) => {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        if (actionUrl) {
          setToast({ type: 'success', text: data.message || 'Payment link opened in new tab, link copied to clipboard' })
          return
        }
  ...
  return (
    <div className="relative flex min-w-[136px] items-center justify-end gap-1.5 whitespace-nowrap">
          ...
          {open && typeof document !== 'undefined' && createPortal(
            ...
            >
              ...
              <button
                onClick={() => {
                  setOpen(false)
                  if (confirm(`Confirm deletion of ${acc.email}?`)) {
      ...
      {actions.length === 0 && (
        <button
          onClick={() => { if (confirm(`Confirm deletion of ${acc.email}?`)) apiFetch(`/accounts/${acc.id}`, { method: 'DELETE' }).then(onDelete) }}
            ...
            <div className="space-y-2">
              ...
              {verificationMailbox?.email && (
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                  Captcha Email: {verificationMailbox.email} · {verificationMailbox.provider || '-'} · ID {verificationMailbox.account_id || '-'}
                </div>
              )}
            </div>
          )}
      ...
      {open && (
        <div className="absolute right-0 top-10 z-20 min-w-[148px] rounded-lg border border-[var(--border)] bg-[var(--bg-card)] py-1 shadow-lg">
          <div className="px-3 py-1 text-[11px] text-[var(--text-muted)]">
            {hasSelection ? `Export ${selectedIds.length} selected accounts` : 'Export current results'}
          </div>
          {options.map(option => (
            <button
              key={option.key}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-5 py-4 border-b border-[var(--border)]/50">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 text-xs">
              <span className="text-[var(--text-muted)]">Total {total} accounts</span>
          ...
          <div className="flex items-center gap-2">
            ...
            {selectedCount > 0 && (
              <Button
                ...
                onClick={async () => {
                  if (!confirm(`Confirm deletion of the selected ${selectedCount} accounts? This action cannot be undone.`)) return
              ...
              <tr>
                <td colSpan={7} className="px-4 py-24 text-center">
                  <div className="flex flex-col items-center justify-center space-y-3">
                    ...
                    <p className="text-xs text-[var(--text-muted)] max-w-sm">No accounts found for this platform. You can add them manually or via bulk import.</p>
                  </div>
                </td>
              </tr>
            )}
